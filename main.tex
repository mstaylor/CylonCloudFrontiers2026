...
\begin{figure}[ht]
    ...
    \end{center}
    \caption{AllReduce Latency vs. Message Size. This figure shows the latency of the AllReduce collective operation across various message sizes for different AWS Lambda invocations.}
    \label{fig:allreducelatency}
\end{figure}
...
\begin{figure}[ht]
    ...
    \end{center}
    \caption{Barrier Latency vs. Node Count. This figure shows the scaling of barrier synchronization latency as the number of AWS Lambda function invocations increases.}
    \label{fig:barrierlatencyscaling}
\end{figure}

Figure \ref{fig:barrierlatencyscaling} shows barrier latency scaling with node count on AWS Lambda. Barrier latency scales with $\log_2(N)$ as expected for a binomial tree algorithm, confirming that synchronization overhead remains in the single-digit millisecond range, suitable for BSP workloads requiring frequent barriers.
