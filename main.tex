...
Similar to the Cylon library, the Twister2 toolkit developed by Kamburugamuwe et al. is designed using the Bulk Synchronous Parallel (BSP) architecture, based on the observed advantages of scalability and performance. It also incorporates a dataframe API implemented in Java\cite{perera2023depth}. While MPI implementations encompass scalable and efficient communication routines, their process launching mechanisms work well with mainstream HPC systems but are incompatible with some environments that adopt their own resource management systems \cite{shan2022hybrid}. A crucial abstraction introduced by Twister2 is TSets, a concept similar to dataframes or, equivalently, RDDs in Apache Spark or datasets in Apache Flink. Twister2 is considered a foundational step towards high-performance data engineering research and serves as a direct precursor to Cylon\cite{pererathesis} and earlier work, Harp-DAAL~\cite{Harp}, a collective communication library that integrates HPC and big data frameworks to support machine learning applications and Twister \cite{ekanayake2010twister}, a runtime for iterative computation that distinguishes between static data (which stays the same across iterations) and dynamic data (which changes each iteration), reducing unnecessary data movement.
...
The execution time for serverless distributed operations can be broken down into three phases: initialization (connection setup via NAT traversal), data generation, and computation. This breakdown provides insight into where time is spent in serverless execution and helps explain performance differences across platforms.
\begin{figure}[ht]
    \begin{center}
    \includegraphics[width=\linewidth]{figure13.jpg}
    \end{center}
    \caption{Per-Execution Cost Analysis. This figure shows the empirical cost breakdown for different operations and communication channels on AWS Lambda, including Lambda compute costs and Step Functions orchestration costs.}
    \label{fig:percostanalysis}
\end{figure}
...
\begin{figure}[ht]
    ...
    \end{center}
    \caption{AllReduce weak scaling on AWS Lambda. This figure shows average AllReduce execution time as parallelism increases for AWS Lambda with 10GB memory.}
    \label{fig:allreducescaling}
\end{figure}
Figure \ref{fig:allreducescaling} shows AllReduce weak scaling results on AWS Lambda with 10GB memory. Average execution time increases from approximately 5 seconds at 1 node to 25.5 seconds at 16 nodes, reflecting the growing communication overhead of the collective operation as parallelism increases.

Figure \ref{fig:percostanalysis} shows the cost breakdown across node counts and operations. Step Functions orchestration cost (shown as the lighter portion) is negligible compared to Lambda compute cost. At 32 nodes, a Join using Redis-mediated communication costs approximately \$0.032, demonstrating the cost-effectiveness of serverless for distributed data operations.

\begin{figure}[ht]
    \begin{center}
    \includegraphics[width=\linewidth]{figure14.jpg}
    \end{center}
    \caption{Per-Execution Cost Comparison by Operation on AWS Lambda. This figure shows the empirical cost for different operations and communication channels across parallelism levels.}
    \label{fig:costbyoperation}
\end{figure}

Figure \ref{fig:costbyoperation} compares per-execution costs across operations using direct TCP communication. Join (DIRECT) incurs the highest cost at 32 nodes, reaching approximately \$0.035, while Microbenchmark (DIRECT) remains the least expensive across all parallelism levels.
...
To provide a clear comparison between the serverless and serverful paradigms, we calculated the estimated cost for a representative distributed join operation on both AWS Lambda and an equivalent number of EC2 instances. Table~\ref{tab:costcomparison} presents the cost for a 32-worker weak scaling join, based on our experimental timing data and on-demand pricing in the \texttt{us-east-1} region as of January 2026.

\begin{table}[ht]
\centering
\caption{Per-Execution Cost Comparison: 32-Worker Weak Scaling Join}
\label{tab:costcomparison}
\small
\begin{tabular}{|l|c|c|c|}
\hline
\textbf{Platform} & \textbf{Config} & \textbf{Time (s)} & \textbf{Cost} \\
\hline
AWS Lambda & 10 GB & 46.56 & \$0.032 \\
\hline
AWS EC2 & m3.xlarge & 41.84 & \$0.238 \\
\hline
\end{tabular}
\vspace{0.5em}
\par\noindent\footnotesize
Lambda cost (USD) = workers $\times$ memory(GB) $\times$ duration(s) $\times$ \$0.0000166667/GB-s \\
EC2 cost (USD) = instances $\times$ hourly\_rate $\times$ duration(h)
\end{table}

The cost for EC2 is calculated based on the on-demand hourly rate for 32 \texttt{m3.xlarge} instances, billed for the duration of the execution. The cost for Lambda is derived from the total GB-seconds consumed by the 32 functions, including initialization and communication overhead. The results show that for this bursty, short-running workload, the serverless approach is approximately \textbf{7.4 times more cost-effective} than provisioning a cluster of virtual machines that would sit idle between executions.

\subsection{Fault Tolerance and Robustness Considerations}
\label{sec:faulttolerance}

Our design, which enables tightly-coupled BSP-style computation in a serverless environment, introduces specific trade-offs in fault tolerance that differ from those of traditional parallel serverless applications. 

The current design is built on the Bulk Synchronous Parallel (BSP) model, in which all workers are expected to complete each superstep synchronously. Consequently, the failure of a single worker (i.e., a Lambda function) during the computation phase will cause the entire distributed task to fail. This is consistent with the behavior of many MPI-based applications in HPC environments, where the loss of a single process often requires a full restart of the job. In our architecture, partial failures are not recoverable without restarting the workflow, as checkpointing mechanisms were not implemented.

However, we have included fault tolerance and resiliency into our design. At the orchestration layer, AWS Step Functions provides robustness by managing the invocations of Lambda functions. Step Functions can be configured with retry policies to automatically re-invoke a Lambda function that fails due to transient issues, such as a temporary network error or a brief service unavailability. This can handle intermittent failures during a function's initialization phase.

At the connection establishment layer, we have implemented several resiliency mechanisms in TCPunch, the NAT traversal component responsible for IP address exchange in the FMI communicator. First, the protocol implements automatic retry logic with a configurable maximum of three protocol-level retries for timeout or error conditions. To maintain the session state over these retries, the server issues reconnect tokens that clients can use to resume a pairing operation without starting from scratch. Second, hole-punching logic employs exponential backoff with jitter when connection attempts fail, preventing thundering herd scenarios when multiple workers retry simultaneously. Third, after a successful hole punch, both peers perform a validation handshake using a magic number verification to ensure the connection was established correctly before proceeding with data transfer. These mechanisms significantly improve the reliability of the connection establishment phase, which is critical in serverless environments where transient network issues are common.

The architecture also relies on two external stateful components: the Rendezvous Server and the Redis instance. While these represent potential single points of failure in a basic deployment, they can be deployed in highly available configurations using standard cloud practices (e.g., load balancers, read replicas, cluster mode) to mitigate this risk.

In summary, our design prioritizes achieving high performance for communication-intensive workloads by trading the stateless, independent-failure model of conventional serverless computing for the performance benefits of a coupled, BSP-style architecture. This makes the system well-suited for performance-critical batch processing workloads where the cost of an occasional full restart is acceptable. As noted in our Future Work section, we plan to enhance the system's resilience by implementing checkpointing and recovery mechanisms inspired by frameworks such as Twister2, enabling recovery of long-running jobs from partial failures.
