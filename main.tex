...
Our design, which enables tightly-coupled BSP-style computation in a serverless environment, introduces specific trade-offs regarding fault tolerance that differ from traditional, embarrassingly parallel serverless applications. We explicitly acknowledge these trade-offs and their implications for the system's robustness.

The current implementation adheres to the classic Bulk Synchronous Parallel (BSP) model, where all workers are expected to complete each super-step synchronously. Consequently, the failure of a single worker (i.e., a Lambda function) during the computation phase will lead to the failure of the entire distributed task. This is consistent with the behavior of many MPI-based applications in HPC environments, where the loss of a single process often requires a full restart of the job. In our architecture, partial failures are not recoverable without restarting the workflow, as we have not yet implemented checkpointing mechanisms.

However, the system is not without any resilience. At the orchestration layer, AWS Step Functions provides a degree of robustness by managing the invocation of the Lambda functions. Step Functions can be configured with retry policies to automatically re-invoke a Lambda function that fails due to transient issues, such as a temporary network error or a brief service unavailability. This can handle intermittent failures during the initialization phase of a function before it joins the main computation.

At the connection establishment layer, we have implemented several resiliency mechanisms in TCPunch, the NAT traversal component responsible for IP address exchange in the FMI communicator. First, the protocol implements automatic retry logic with a configurable maximum of three protocol-level retries for timeout or error conditions. To maintain session state across these retries, the server issues reconnect tokens that clients can use to resume a pairing operation without starting from scratch. Second, the hole-punching logic employs exponential backoff with jitter when connection attempts fail, preventing thundering herd scenarios when multiple workers retry simultaneously. Third, after a successful hole punch, both peers perform a validation handshake using a magic number verification to ensure the connection was established correctly before proceeding with data transfer. These mechanisms significantly improve the reliability of the connection establishment phase, which is critical in serverless environments where transient network issues are common.
