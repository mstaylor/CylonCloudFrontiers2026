...
\textbf{(iii) Quantitative Cost-Performance Analysis for Serverless HPC:} We present an empirical cost model for data-intensive serverless workloads. Our analysis reveals that serverless computing can be cost-competitive for bursty workloads. For instance, a 32-worker distributed join operation on Lambda incurs approximately \$0.21 with pay-per-millisecond billing, whereas provisioned EC2 instances have idle time as the primary cost driver for intermittent workloads.
