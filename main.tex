...
\textbf{(i) BSP Execution Model for Serverless Computing:} We demonstrate that Bulk Synchronous Parallel (BSP) workloads---traditionally requiring dedicated HPC infrastructure with low-latency interconnects---can execute effectively in serverless environments. By enabling direct peer-to-peer communication between Lambda functions via NAT traversal, we show that parallel computations achieve scaling efficiency within 6.5\% of that observed on provisioned cloud VMs. This challenges the assumption that serverless is limited to embarrassingly parallel or loosely coupled workloads.

\textbf{(ii) Unified Framework for ML Data Pipelines Across Computing Paradigms:} We present Cylon as a portable, distributed dataframe library that enables the same data-intensive operations to run across serverless (AWS Lambda), cloud VM (EC2), and HPC (Rivanna) environments without significant code modification. This supports both ML training and inference pipelines. Scientific domains with large-scale data processing needs---such as genomics, hydrology, and astronomy---could leverage Cylon's distributed operations in serverless deployments, enabling elastic scaling for bursty workloads without dedicated infrastructure.

\textbf{(iii) Quantitative Cost-Performance Analysis for Serverless HPC:} We present an empirical cost model for data-intensive serverless workloads. Our analysis shows that serverless can be cost-competitive for bursty workloads: a 32-worker distributed join costs approximately \$0.21 on Lambda with pay-per-millisecond billing, compared with provisioned EC2 instances, where idle time dominates cost for intermittent workloads.

\textbf{(iv) Communication Substrate Comparison for Serverless Collectives:} We evaluate three communication approaches for MPI-style collectives in serverless environments: direct TCP via NAT hole-punching, Redis, and AWS S3 message passing. Our results show that direct communication achieves 10-100$\times$ lower latency than storage-mediated alternatives, establishing NAT traversal as a viable approach for latency-sensitive distributed computing in serverless environments.
...
\begin{figure}[ht]
    \begin{center}
    \includegraphics[width=\linewidth, height=200pt]{figure4.jpg}
    \end{center}
...
\begin{lstlisting}[style=pythonstyle, float, caption=Python Scaling Join Experiment]
import pandas as pd
...
Figure \ref{fig:groupbyscaling} shows GroupBy weak scaling results on Lambda with 9.1 million rows per node. The operation scales from approximately 5 seconds at 1 node to 27 seconds at 32 nodes, following a similar scaling pattern to Join. The consistent performance with small error bars demonstrates that shuffle-based distributed operators work reliably in serverless environments.
